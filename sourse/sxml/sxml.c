...
static BOOL state_pushtoken (sxml_t* state, sxml_args_t* args, sxmltype_t type, const char* start, const char* end, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	sxmltok_t __token__ = {0};
	sxmltok_t* token = &__token__;
	UINT i= state->ntokens++;
	if (args->num_tokens != 0 && args->num_tokens < state->ntokens)
		return FALSE;
	...
	addTockenCallBack(token, addTockenCallBackData);
...
static sxmlerr_t state_setpos (sxml_t* state, const sxml_args_t* args, const char* ptr)
{
	...
	return (args->num_tokens == 0 || state->ntokens <= args->num_tokens) ? SXML_SUCCESS : SXML_ERROR_TOKENSFULL;
}
...
static sxmlerr_t parse_characters (sxml_t* state, sxml_args_t* args, const char* end, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	...
	if (ampr != start)
		state_pushtoken (state, args, SXML_CHARACTER, start, ampr, addTockenCallBack, addTockenCallBackData);
	...
	start= colon + 1;
	state_pushtoken (state, args, SXML_CHARACTER, ampr, start, addTockenCallBack, addTockenCallBackData);
	return state_setpos (state, args, start);
}

static sxmlerr_t parse_attrvalue (sxml_t* state, sxml_args_t* args, const char* end, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	while (buffer_fromoffset (args, state->bufferpos) != end)
	{
		sxmlerr_t err= parse_characters (state, args, end, addTockenCallBack, addTockenCallBackData);
		if (err != SXML_SUCCESS)
			return err;
	}
...
static sxmlerr_t parse_attributes (sxml_t* state, sxml_args_t* args, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	const char* start= buffer_fromoffset (args, state->bufferpos);
	...
	while (name != end && ISALPHA (*name))
	{
		...
		state_pushtoken (state, args, SXML_CDATA, name, space, addTockenCallBack, addTockenCallBackData);
		...
		err= parse_attrvalue (state, args, quot, addTockenCallBack, addTockenCallBackData);
		if (err != SXML_SUCCESS)
			return err;
...
static sxmlerr_t parse_comment (sxml_t* state, sxml_args_t* args, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	...
	state_pushtoken (state, args, SXML_COMMENT, start, dash, addTockenCallBack, addTockenCallBackData);
	return state_setpos (state, args, dash + TAG_LEN (ENDTAG));
}

static sxmlerr_t parse_instruction (sxml_t* state, sxml_args_t* args, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	static const char STARTTAG[]= "<?";
	static const char ENDTAG[]= "?>";
	...
	state_pushtoken (state, args, SXML_INSTRUCTION, start, space, addTockenCallBack, addTockenCallBackData);

	state_setpos (state, args, space);
	err= parse_attributes (state, args, addTockenCallBack, addTockenCallBackData);
	if (err != SXML_SUCCESS)
		return err;
...
static sxmlerr_t parse_doctype (sxml_t* state, sxml_args_t* args, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	...
	state_pushtoken (state, args, SXML_DOCTYPE, start, bracket, addTockenCallBack, addTockenCallBackData);
	return state_setpos (state, args, bracket + TAG_LEN (ENDTAG));
}

static sxmlerr_t parse_start (sxml_t* state, sxml_args_t* args, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{	
	sxmlerr_t err;
	...
	state_pushtoken (state, args, SXML_STARTTAG, name, space, addTockenCallBack, addTockenCallBackData);
	...
	if (gt != end && *gt == '/')
	{
		state_pushtoken (state, args, SXML_ENDTAG, name, space, addTockenCallBack, addTockenCallBackData);
		gt++;
	}
...
static sxmlerr_t parse_end (sxml_t* state, sxml_args_t* args, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	const char* gt, *space;
	const char* start= buffer_fromoffset (args, state->bufferpos);
	const char* end= buffer_getend (args);
	...
	state_pushtoken (state, args, SXML_ENDTAG, start, space, addTockenCallBack, addTockenCallBackData);
	return state_setpos (state, args, gt + 1);
}

static sxmlerr_t parse_cdata (sxml_t* state, sxml_args_t* args, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	...
	state_pushtoken (state, args, SXML_CDATA, start, bracket, addTockenCallBack, addTockenCallBackData);
	return state_setpos (state, args, bracket + TAG_LEN (ENDTAG));
}
...
sxmlerr_t sxml_parse(sxml_t *state, const char *buffer, UINT bufferlen, sxmltok_t tokens[], UINT num_tokens, AddTockenCallBack addTockenCallBack, void* addTockenCallBackData)
{
	sxml_t temp= *state;
	...
	while (!ROOT_FOUND (&temp))
	{
		...
		switch (lt[1])
		{
		case '?':	err= parse_instruction (&temp, &args, addTockenCallBack, addTockenCallBackData);	break;
		case '!':	err= parse_doctype (&temp, &args, addTockenCallBack, addTockenCallBackData);	break;
		default:	err= parse_start (&temp, &args, addTockenCallBack, addTockenCallBackData);	break;
		}
	...
	while (!ROOT_PARSED (&temp))
	{
		...
		{
			sxmlerr_t err= parse_characters (&temp, &args, lt, addTockenCallBack, addTockenCallBackData);
			if (err != SXML_SUCCESS)
				return err;
		...
		{
		case '?':	err= parse_instruction (&temp, &args, addTockenCallBack, addTockenCallBackData);		break;
		case '/':	err= parse_end (&temp, &args, addTockenCallBack, addTockenCallBackData);	break;
		case '!':	err= (lt[2] == '-') ? parse_comment (&temp, &args, addTockenCallBack, addTockenCallBackData) : parse_cdata (&temp, &args, addTockenCallBack, addTockenCallBackData);	break;
		default:	err= parse_start (&temp, &args, addTockenCallBack, addTockenCallBackData);	break;
		}
